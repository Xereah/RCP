                ...
                <div class="flex items-center space-x-5">
                    <div class="relative">
                        ...
                        <div class="absolute -top-1 -right-1 w-6 h-6 bg-emerald-500 rounded-full flex items-center justify-center">
                            <span class="text-xs font-semibold text-white">{{ count($workPlaces) }}</span>
                        </div>
                    </div>
                    <div>
                        ...
                        <p class="text-lg text-gray-600">Zarządzaj miejscami pracy</p>
                    </div>
                </div>
                
                <!-- CTA Button -->
                <div class="flex-shrink-0">
                    <a href="{{ route('work-places.create') }}" 
                       wire:navigate
                ...
                <table class="w-full">
                    ...
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-900 uppercase tracking-wider">Nazwa miejsca pracy</th>                            
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($workPlaces as $key => $workPlace)
                            <tr class="hover:bg-gray-50 transition-colors duration-150" wire:key="{{ $workPlace->id }}">
                                <!-- Row Number -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center">
                                            <span class="text-sm font-semibold text-gray-700">
                                                {{ ($workPlaces->currentPage() - 1) * $workPlaces->perPage() + $loop->iteration }}
                                            </span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('work-places.edit', $workPlace->id) }}" 
                                           wire:navigate
                                           ...
                                           title="Edytuj miejsce pracy">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                            </svg>
                                        </a>
                                        <button @click="confirmAction('delete', {{ $workPlace->id }}, 'Na pewno chcesz usunąć to miejsce pracy?', 'Tak, usuń!', 'warning')"
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            Pokazano <span class="font-semibold">{{ $workPlaces->firstItem() }}</span> - 
                            <span class="font-semibold">{{ $workPlaces->lastItem() }}</span> z 
                            <span class="font-semibold">{{ $workPlaces->total() }}</span> wyników
                        </div>
                        <div class="flex space-x-1">
                            {!! $workPlaces->withQueryString()->links() !!}
                        </div>
                    </div>
                </div>
            @endif
