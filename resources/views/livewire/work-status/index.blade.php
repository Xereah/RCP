                ...
                <div class="flex items-center space-x-5">
                    <div class="relative">
                        ...
                        <div class="absolute -top-1 -right-1 w-6 h-6 bg-emerald-500 rounded-full flex items-center justify-center">
                            <span class="text-xs font-semibold text-white">{{ count($workStatuses) }}</span>
                        </div>
                    </div>
                    <div>
                        ...
                        <p class="text-lg text-gray-600">Zarządzaj statusami obecności</p>
                    </div>
                </div>
                ...
                <table class="w-full">
                    ...
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-900 uppercase tracking-wider">Nazwa statusu</th>                            
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($workStatuses as $key => $workStatus)
                            <tr class="hover:bg-gray-50 transition-colors duration-150" wire:key="{{ $workStatus->id }}">
                                <!-- Row Number -->
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center">
                                            <span class="text-sm font-semibold text-gray-700">
                                                {{ ($workStatuses->currentPage() - 1) * $workStatuses->perPage() + $loop->iteration }}
                                            </span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('work-statuses.edit', $workStatus->id) }}" 
                                           wire:navigate
                                           ...
                                           title="Edytuj status obecności">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                            </svg>
                                        </a>
                                        <button @click="confirmAction('delete', {{ $workStatus->id }}, 'Na pewno chcesz usunąć ten status obecności?', 'Tak, usuń!', 'warning')"
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            Pokazano <span class="font-semibold">{{ $workStatuses->firstItem() }}</span> - 
                            <span class="font-semibold">{{ $workStatuses->lastItem() }}</span> z 
                            <span class="font-semibold">{{ $workStatuses->total() }}</span> wyników
                        </div>
                        <div class="flex space-x-1">
                            {!! $workStatuses->withQueryString()->links() !!}
                        </div>
                    </div>
                </div>
            @endif
