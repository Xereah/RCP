<div class="min-h-screen bg-gradient-to-br from-gray-50 to-slate-100 py-4 px-4" 
    ...
    <div class="mx-auto">
                ...
                <div class="flex items-center space-x-5">
                    <div class="relative">
                        ...
                        <div class="absolute -top-1 -right-1 w-6 h-6 bg-emerald-500 rounded-full flex items-center justify-center">
                            <span class="text-xs font-semibold text-white">{{ count($users) }}</span>
                        </div>
                    </div>
                    <div>
                        ...
                        <p class="text-lg text-gray-600">Zarządzaj użytkownikami systemu</p>
                    </div>
                </div>
        ...
        <!-- Pole Livewire LiveSearch -->
        <div class="mb-4">
            <input type="text" wire:model.live="search" placeholder="Wyszukaj użytkownika" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-indigo-500">
        </div>
                ...
                <table class="w-full">
                    ...
                    <thead class="bg-gray-50 border-b border-gray-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-900 uppercase tracking-wider">Rola</th> 
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($users as $key => $user)
                            <tr class="hover:bg-gray-50 transition-colors duration-150" wire:key="{{ $user->id }}">
                                <!-- Row Number -->
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center space-x-2">
                                        ...
                                        <button @click="confirmAction('delete', {{ $user->id }}, 'Na pewno chcesz usunąć tego użytkownika?', 'Tak, usuń!', 'warning')"
                ...
                <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            Pokazano <span class="font-semibold">{{ $users->firstItem() }}</span> - 
                            <span class="font-semibold">{{ $users->lastItem() }}</span> z 
                            <span class="font-semibold">{{ $users->total() }}</span> wyników
                        </div>
